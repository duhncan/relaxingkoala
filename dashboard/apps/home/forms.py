...
from wtforms import StringField, SubmitField, FloatField, HiddenField, DateTimeField, SelectMultipleField, IntegerField, SelectField, FileField
...
class PaymentForm(FlaskForm):
    ...
    payment_type = SelectField('Payment Type', choices=[('cash', 'Cash'), ('card', 'Card')], validators=[DataRequired()])
...
class CardPaymentForm(PaymentForm):
    ...
    card_expiration_date = StringField('Card Expiration Date (MM/YYYY)', validators=[DataRequired(), Length(min=7, max=7)])
...
class OrderSearchForm(FlaskForm):
    search_query = StringField('Enter your name, email, or phone number', validators=[DataRequired()])
